...
class TunnelService : VpnService() {
    ...
    companion object {
        ...
        private const val NOTIFICATION_CHANNEL_ID = "tunnel_vpn"
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_STOP -> {
                stopTunnel()
                // FIX (found via real-device testing): relying on
                // stopSelf() alone to also tear down the foreground
                // notification is unreliable across Android versions --
                // the notification stuck around until a NEW VPN start
                // overwrote it. Explicitly remove it here.
    ...
    private fun startTunnel() {
        ...
        try {
            ...
            android.util.Log.e("TunnelService", "startVpnTunnel failed: ${e.message}", e)
            tunFd = null
            stopForeground(STOP_FOREGROUND_REMOVE)
            stopSelf()
        }
